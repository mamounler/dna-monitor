...
A small script I wrote to learn how dark web threat collection actually works. It connects to a Tor-hosted forum, crawls a target section across multiple pages, flags threads matching keywords I care about (credentials, carding, malware, etc), then goes back to each flagged thread to grab the author, post date, and a preview.
...
- Don't follow external links. Without a domain check, a crawler ends up trying to fetch Twitter and random `.onion` mirrors through Tor for nothing. A simple `is_internal()` filter keeps it on scope.
...
Output:
```
101 threads, 45 alerts
```
Output goes to `data/alerts/alerts.json`.